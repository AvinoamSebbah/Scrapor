...
class ShortTermDBDatasetManager:
    ...
    @staticmethod
    def _normalize_scraper_token(value: str) -> str:
        if not value:
            return ""
        return "".join(ch for ch in value.lower() if ch.isalnum())

    def _is_scraper_enabled_for_name(self, name: str) -> bool:
        if not self.enabled_scrapers:
            return True
        normalized_name = self._normalize_scraper_token(name)
        for scraper in self.enabled_scrapers:
            normalized_scraper = self._normalize_scraper_token(scraper)
            if normalized_scraper and normalized_scraper in normalized_name:
                return True
        return False
    ...
    def _push_status_files(self, local_cahce: CacheState):
        for file in os.listdir(self.status_folder):
            ...
            if not self._is_scraper_enabled_for_name(file_base):
    ...
    def _push_files_data(self, local_cahce: CacheState):
        ...
        if hasattr(self.uploader, "get_processed_files_names"):
            ...
            Logger.info(f"PostgreSQL sync: Found {len(processed_files)} processed files in DB.")
        ...
        for file in sorted(os.listdir(self.outputs_folder), key=lambda x: ("store" not in x.lower(), x)):
            ...
            if not self._is_scraper_enabled_for_name(file):
        ...
        try:
            with CacheManager(self.app_folder) as local_cache:
                if local_cache.is_empty() or force_restart:
                    ...
                    # If the uploader supports syncing cache from DB (like PostgreSQL), do it here
                ...
                if hasattr(self.uploader, "maybe_analyze_products"):
                    self.uploader.maybe_analyze_products()
        finally:
            if hasattr(self.uploader, "close"):
                self.uploader.close()
