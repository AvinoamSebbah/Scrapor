...
class BaseSupermarketDataPublisher:
    ...
    def _load_processed_files_cache_file(self):
        """Load processed_files metadata from PROCESSED_FILES_CACHE JSON when available."""
        cache_file = os.getenv("PROCESSED_FILES_CACHE")
        if not cache_file:
            return []
        if not os.path.exists(cache_file):
            Logger.warning("PROCESSED_FILES_CACHE file not found: %s", cache_file)
            return []
        ...
        try:
            ...
            Logger.warning("Failed to read PROCESSED_FILES_CACHE file %s: %s", cache_file, e)
            return []
        ...
        for row in rows:
            ...
            if not file_name:
                continue
            names.add(file_name)
            metadata.append({"file_name": file_name, "chain_name": row.get("chain_name")})

        if names:
            self.processed_files_cache = names
            Logger.info("Loaded %d processed files from cache file", len(names))
    ...
    def _execute_scraping(self):
        ...
        try:
            ...
            if hasattr(self.short_term_db_target, "get_processed_files_metadata"):
                metadata = self.short_term_db_target.get_processed_files_metadata() or []
            if not metadata:
                metadata = self._load_processed_files_cache_file()
            ...
            if metadata:
                ...
                for scraper_name in self.enabled_scrapers:
                    try:
                        ...
                        # Keep a canonical fallback under app_data/dumps to avoid
                        # path mismatches between scraper internals and local runs.
                        storage_basename = os.path.basename(os.path.normpath(storage_path))
                        if storage_basename:
                            candidate_paths.add(os.path.join(self.data_folder, storage_basename))

                        for path in sorted(candidate_paths):
                            os.makedirs(path, exist_ok=True)
                            all_storage_paths.append(path)
                        Logger.info(
                            f"[marker-prep] scraper={scraper_name} -> folders={sorted(candidate_paths)}"
                        )
                    except Exception as e:
                ...
                for item in metadata:
                    ...
                    for storage_path in all_storage_paths:
                        # Create markers for extensionless names (Bina/Aspx scraper
                        # comparison), compressed (.gz) and plain (.xml) variants.
                        for marker_name in (base_name, base_name + ".gz", f_name):
                ...
                Logger.warning(
                    "SCRAPE SKIP SUMMARY: prepared %d marker files from processed_files cache for scrapers=%s",
                    skipped_prep_count,
                    ",".join(self.enabled_scrapers) if self.enabled_scrapers else "all",
                )
    ...
    def _sync_remote_state(self):
        ...
        if hasattr(self.short_term_db_target, "get_processed_files_names"):
            Logger.info("Syncing remote state from PostgreSQL/cache...")
        ...
        self._load_processed_files_cache_file()
            ...
            for root, _, files in os.walk(self.data_folder):
                for file in files:
                    ...
                    if file in self.processed_files_cache or is_marker:
                        ...
                        skipped_count += 1
            if skipped_count > 0:
                Logger.info("Skipped conversion for %d files already present in processed_files.", skipped_count)
                Logger.warning(
                    "SCRAPE SKIP SUMMARY: skipped %d files already present in processed_files",
                    skipped_count,
                )
