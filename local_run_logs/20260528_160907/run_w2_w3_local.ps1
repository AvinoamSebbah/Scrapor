...
$SummaryPath = Join-Path $LogDir 'pipeline_summary.csv'
...
for ($i = 0; $i -lt $stores.Count; $i++) {
  ...
  if ($w2Exit -eq 0 -and $hasUploadData) {
    ...
    $w3Exit = Invoke-LoggedStep -Title "W3 Upload $store -> PostgreSQL" -LogPath $w3Log -Body {
      python (Join-Path $LogDir 'run_main_local.py')
    }
  } elseif ($w2Exit -eq 0) {
...
Write-State -Phase 'complete' -Store '' -Index $total -Total $total -Status 'completed' -Message 'all stores processed'
exit 0
