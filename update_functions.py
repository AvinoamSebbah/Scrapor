...
def update_functions():
  ...
  try:
    ...
    with conn.cursor() as cur:
        ...
        """)
        print("✅ enforce top_promotions_cache all-time window")
        ...
        """)
        print("✅ ensure top_promotions_cache filtered-chain index")
          ...
          WITH scoped_store_promos AS (
            SELECT
              ...
              ROUND(
                ...
                - (
                  CASE
                    WHEN s.chain_id = '7290172900007'
                      OR COALESCE(s.chain_name, '') = 'סופר פארם ישראל'
                    THEN 1000
                    ELSE 0
                  END
                ),
            ...
            WHERE (
              ...
              AND p.item_name NOT ILIKE ANY(ARRAY[
                ...
                '%סיגריה%','%סיגריות%','%טבק%','%סיגר%','%ניקוטין%','%אלכוהול%',
                '%שיבאס%','%גלנליווט%','%גים בים%',
                '%ט.קוארבו%','%אוזו%','%פלומרי%',
                '%פרלמנט%','%קנט%','%מלבורו%','%וינסטון%',
                '%פאל מאל%','%קופסה אירופה%','%מונלייט%',
                '%ג''ק דניאלס%','%ג׳ק דניאלס%',
